...
test.describe('Sprint 2: Promotion Distribution System (14 Cases)', () => {
  ...
  test('TC-VIEW-001: Retailer views active promotions list', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText('Promotions Hub');
    const cards = page.locator('.bg-white.rounded-\\[28px\\]'); 
  ...
  test('TC-VIEW-002: Promotions show correct title and description', async ({ page }) => {
    ...
    const firstDesc = page.locator('p.text-gray-500').first();
  ...
  test('TC-VIEW-003: Promotions show validity date range', async ({ page }) => {
    await login(page, accounts.retailer1.email, password);
    await page.goto(`${baseUrl}/retailer/promotions`);
    await expect(page.locator('text=Ends').first()).toBeVisible();
  });

  test('TC-VIEW-004: Search functionality filters promotions list', async ({ page }) => {
    await login(page, accounts.retailer1.email, password);
    await page.goto(`${baseUrl}/retailer/promotions`);
    await page.fill('input[placeholder="Search bulk offers..."]', 'NonExistentPromoXYZ');
  ...
  test('TC-CREATE-001: PM creates promotion with valid details', async ({ page }) => {
    ...
    await page.goto(`${baseUrl}/promotion-manager/create-b2b`);
    const title = `QA Promo ${Date.now()}`;
    await page.fill('input[placeholder*="Nescafé Bulk"]', title);
    await page.fill('textarea[placeholder*="Explain the promotion"]', 'Standard automated test promotion');
    await page.fill('input[placeholder="15"]', '15');
    await page.fill('input[type="datetime-local"] >> nth=0', '2026-06-01T12:00');
    await page.fill('input[type="datetime-local"] >> nth=1', '2026-12-31T12:00');
