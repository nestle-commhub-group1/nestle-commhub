...
const ManagerLayout = ({ children }) => {
  ...
  const navItems = [
    ...
    { label: 'Regional Tickets', path: '#', icon: <FileText size={20} /> },
    { label: 'Escalated Tickets', path: '#', icon: <AlertCircle size={20} /> },
    { label: 'My Sales Staff', path: '#', icon: <Users size={20} /> },
    { label: 'Analytics', path: '#', icon: <BarChart2 size={20} /> },
  ...
  const getNotificationIcon = (type) => {
    switch(type) {
      ...
      case 'warning': return <div className="text-yellow-500"><AlertCircle size={16} /></div>;
      case 'success': return <div className="text-green-500"><CheckCircle size={16} /></div>;
  ...
  const TopBar = () => (
    <div className="flex bg-nestle-brown text-white h-16 items-center justify-between px-4 lg:hidden sticky top-0 z-20">
      ...
      <button className="p-2 relative" onClick={() => setIsNotificationsOpen(true)}>
        <Bell size={24} />
        <span className="absolute top-1 right-1 bg-nestle-danger text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full min-w-[16px] text-center">3</span>
      </button>
    </div>
  );
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-72 bg-nestle-brown text-white transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="px-6 py-4 flex items-center space-x-4 mb-2">
          ...
          <div className="flex flex-col overflow-hidden">
            ...
            <span className="bg-[#F3E8FF] text-[#6B21A8] text-[10px] font-bold px-2 py-0.5 rounded-full w-max mt-1 tracking-wide border border-[#6B21A8]/20">
              Regional Manager
            </span>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-1.5 pb-4">
          {navItems.map((item, idx) => {
            const isActive = location.pathname === item.path || (item.label === 'Home' && location.pathname === '/manager/dashboard');
            return item.path === '#' ? (
              <button 
                key={idx} 
                onClick={item.action}
