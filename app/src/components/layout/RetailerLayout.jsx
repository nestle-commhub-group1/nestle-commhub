...
import {
  Home, FileText, Bell, User, LogOut, Menu, X, ChevronRight, CheckCircle, Package, PiggyBank, Truck, Plus, Tag, LayoutDashboard, Loader2
} from 'lucide-react';
...
const RetailerLayout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (storedUser) {
      ...
      setUser({ fullName: 'Chaminda Jayawardena', initials: 'CJ', role: 'Retailer', email: 'chaminda@samanstores.lk' });
    }
    fetchNotifications();
  }, []);
  ...
  const navItems = [
    { label: 'Home', path: '/retailer/dashboard', icon: <LayoutDashboard size={20} /> },
    { label: 'My Tickets', path: '/retailer/tickets', icon: <FileText size={20} /> },
  ...
  const getNotificationIcon = (type) => {
    switch (type) {
      ...
      case 'promotion': return <Tag size={16} className="text-purple-500" />;
      ...
      <div className={`fixed inset-y-0 left-0 z-40 w-72 bg-nestle-brown text-white transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 flex flex-col ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-1.5 pb-4">
          {navItems.map((item, idx) => {
            const isActive = location.pathname === item.path || (item.label === 'Home' && location.pathname === '/retailer/dashboard');
            return item.path === '#' ? (
              <button
                key={idx}
                onClick={item.action}
