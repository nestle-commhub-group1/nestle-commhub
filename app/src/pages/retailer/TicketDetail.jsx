...
import { ArrowLeft, Send, Clock, User, Paperclip, Loader2, AlertCircle } from 'lucide-react';
...
export default function TicketDetail() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [ticketRes, msgRes] = await Promise.all([
          axios.get(`http://localhost:5001/api/tickets/${id}`, { headers: { Authorization: `Bearer ${token}` } }),
          axios.get(`http://localhost:5001/api/tickets/${id}/messages`, { headers: { Authorization: `Bearer ${token}` } })
        ]);
  ...
  const send = async () => {
    ...
    try {
      setSending(true);
      const res = await axios.post(`http://localhost:5001/api/tickets/${id}/messages`, {
        message: input.trim()
      }, {
              ...
              <div className="bg-white border border-[#E0DBD5] rounded-[20px] p-6 shadow-sm">
                ...
                <div className="flex gap-3 flex-wrap">
                  {ticket.attachments.map((url, i) => (
                    <a key={i} href={url} target="_blank" rel="noopener noreferrer" className="w-20 h-20 bg-[#F5F3F0] border border-[#E0DBD5] rounded-[10px] flex items-center justify-center text-2xl overflow-hidden">
            ...
            <div className="bg-white border border-[#E0DBD5] rounded-[20px] p-6 shadow-sm">
              ...
              <div className="space-y-0">
                {[
                  ...
                  { label: 'In Progress', done: ['in_progress', 'resolved', 'escalated'].includes(ticket.status), active: ticket.status === 'in_progress', time: ticket.status === 'in_progress' ? 'Current' : '' },
                  { label: 'Resolved', done: ticket.status === 'resolved', active: false, time: ticket.resolvedAt ? new Date(ticket.resolvedAt).toLocaleString() : 'Pending' },
                ].map((step, idx, arr) => (
                  <div key={idx} className="flex items-start space-x-3">
                    <div className="flex flex-col items-center">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${step.done ? 'bg-[#2D7A4F]' : step.active ? 'bg-[#2563EB]' : 'bg-gray-200'}`}>
                        {step.done && <span className="text-white text-[12px]">✓</span>}
                        {step.active && <span className="w-2.5 h-2.5 bg-white rounded-full animate-pulse block"/>}
                      </div>
                      {idx < arr.length - 1 && <div className={`w-0.5 h-8 my-1 ${step.done ? 'bg-[#2D7A4F]' : 'bg-gray-200'}`}/>}
                    </div>
                    <div className="pb-3">
                      <p className={`text-[14px] font-bold ${step.done || step.active ? 'text-[#2C1810]' : 'text-gray-400'}`}>{step.label}</p>
                      <p className="text-[12px] text-gray-500 mt-0.5">{step.time}</p>
                    </div>
                  </div>
                ))}
              </div>
