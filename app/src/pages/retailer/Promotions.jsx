...
const Promotions = () => {
  ...
  const impactScore = ratingData.score || 5;
  const impactUnits = Number(ratingData.unitsSold) || 0;
  const estimatedReward = Math.round(impactUnits * 0.5);
    ...
    <RetailerLayout>
      ...
      <div className="space-y-6 pb-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center pt-2">
          <div>
            <h1 className="text-[28px] font-black text-[#2C1810] tracking-tight">{t('Promotions Hub')}</h1>
            <p className="text-[13px] text-gray-500 font-medium mt-1">
              {activeTab === 'wall' && t('Direct bulk offers from Nestlé to grow your inventory')}
              {activeTab === 'smart' && t("Personalised recommendations based on your store's success")}
              {activeTab === 'customer' && t('Official activations for your digital storefront')}
            </p>
          </div>
          ...
          <div className="mt-4 md:mt-0 flex items-center space-x-3">
             ...
             {activeTab === 'wall' && (
               <div className="bg-white px-4 py-2 rounded-2xl border border-[#E0DBD5] flex items-center space-x-3 shadow-sm">
                 ...
                 <div>
                   <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">{t('Points')}</p>
        ...
        <div className="flex p-1 bg-white border border-[#E0DBD5] rounded-[20px] shadow-sm overflow-x-auto no-scrollbar">
          {[
            { id: 'wall', label: t('Bulk Offers'), icon: <Tag size={16} />, color: 'blue' },
            { id: 'smart', label: t('Smart Picks'), icon: <Sparkles size={16} />, color: 'amber' },
            { id: 'customer', label: t('Customer Deals'), icon: <Users size={16} />, color: 'purple' },
          ].map(tab => (
            <button
              key={tab.id}
        ...
        {activeTab === 'wall' && (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div className="relative max-w-md">
              ...
              <input 
                type="text" 
                placeholder={t('Search bulk offers...')} 
                value={wallSearch}
            ...
            {wallLoading ? (
              <div className="flex flex-col items-center justify-center py-20 text-gray-400 space-y-3">
                ...
                <p className="text-[13px] font-bold">{t('Fetching campaigns...')}</p>
              </div>
            ) : filteredWall.length === 0 ? (
              <div className="bg-white border border-dashed border-[#E0DBD5] rounded-[32px] py-20 text-center">
                <p className="text-4xl mb-4">🏷️</p>
                <p className="text-[16px] font-black text-gray-400">{t('No campaigns found')}</p>
              </div>
            ) : (
                    ...
                    <div key={promo._id} className="bg-white rounded-[28px] p-6 shadow-sm border border-[#E0DBD5] hover:shadow-xl transition-all duration-300 flex flex-col justify-between group">
                      <div>
                        <div className="flex justify-between items-start mb-4">
                          <span className="px-3 py-1 bg-blue-50 text-blue-700 text-[10px] font-black rounded-full uppercase tracking-widest border border-blue-100">
                            {promo.category?.replace('_', ' ') || t('Promo')}
                          </span>
                          {promo.discount && (
                             <span className="flex items-center text-green-600 font-black bg-green-50 px-2.5 py-1 rounded-xl border border-green-100 text-[11px]">
                               <Percent size={12} className="mr-1" /> {promo.discount}% {t('OFF')}
                             </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center text-[12px] text-gray-400 font-bold mb-6">
                          ...
                          <span>{t('Ends')} {formatDate(promo.endDate)}</span>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        {isOptedIn ? (
                          <div className="space-y-2">
                            ...
                            {alreadyRated ? (
                              <div className="flex flex-col items-center pt-1">
                                ...
                                <p className="text-[9px] text-gray-400 font-black uppercase mt-1 tracking-widest">{t('Feedback Sent')}</p>
                              </div>
                            ) : (
                              <button 
                                onClick={() => setRatingData({ id: promo._id, score: 5, feedback: '', unitsSold: '' })}
                              ...
                              >
                                ...
                                <span>{isCompleted ? t('FINAL REVIEW') : t('GIVE FEEDBACK')}</span>
                              </button>
                            )}
                          </div>
                        ) : (
                          ...
                          >
                            {isCompleted ? t('CAMPAIGN ENDED') : t('OPT IN NOW')}
                          </button>
                        )}
          ...
          <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-500">
             <section>
                <div className="flex items-center space-x-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-[17px] font-black text-[#2C1810] uppercase tracking-wider">{t('Past Favourites')}</h2>
                    <p className="text-[12px] text-gray-400 font-medium">{t('Get notified when your top-rated campaigns return')}</p>
                  </div>
                </div>
                ...
                {favLoading ? (
                  ...
                  <div className="bg-white rounded-[32px] border border-dashed border-[#E0DBD5] py-20 text-center">
                    ...
                    <p className="text-[15px] font-black text-gray-400">{t('Your high-rated campaigns will appear here')}</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {favorites.map(promo => (
             ...
             <section>
                <div className="flex items-center space-x-3 mb-6">
                  ...
                  <div>
                    <h2 className="text-[17px] font-black text-[#2C1810] uppercase tracking-wider">{t('Similar To Favourites')}</h2>
                    <p className="text-[12px] text-gray-400 font-medium">{t('Active offers matched to your successful history')}</p>
                  </div>
                </div>
                ...
                {simLoading ? (
                  ...
                  <div className="bg-white rounded-[32px] border border-dashed border-[#E0DBD5] py-20 text-center">
                    <p className="text-4xl mb-4">🎁</p>
                    <p className="text-[15px] font-black text-gray-400">{t('No matches found right now')}</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {similar.map(promo => (
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            {b2cLoading ? (
              <div className="flex flex-col items-center justify-center py-24 space-y-4">
                ...
                <p className="text-[15px] font-bold text-gray-500 tracking-tight">{t('Syncing store offers...')}</p>
              </div>
            ) : b2cPromos.length === 0 ? (
              <div className="bg-white border border-[#E0DBD5] rounded-[40px] p-24 text-center space-y-6 shadow-sm">
                ...
                <div>
                  <h3 className="text-[20px] font-black text-[#2C1810]">{t('No Active Customer Offers')}</h3>
                  <p className="text-[14px] text-gray-400 mt-2 max-w-sm mx-auto font-medium leading-relaxed">
                    {t('Official Nestlé B2C promotions will appear here when available.')}
                  </p>
                </div>
              </div>
            ) : (
                    ...
                    <div className="p-8 lg:p-10">
                      <div className="flex justify-between items-start mb-6">
                         <div className="space-y-1.5">
                            <div className="flex items-center space-x-2">
                               <span className="px-2.5 py-1 bg-purple-50 text-purple-700 text-[10px] font-black rounded-lg uppercase tracking-[0.15em] border border-purple-100 shadow-sm">{t('B2C Deal')}</span>
                               {promo.isActiveInMyStore && <span className="px-2.5 py-1 bg-green-500 text-white text-[10px] font-black rounded-lg uppercase tracking-[0.15em] shadow-md animate-pulse">{t('Active')}</span>}
                            </div>
                      ...
                      <div className="bg-[#3D2B1F] text-white rounded-[24px] p-6 mb-8 shadow-inner relative overflow-hidden group-hover:bg-[#2C1810] transition-colors duration-500">
                        ...
                        <div className="flex items-center justify-between relative z-10">
                          <div className="space-y-1">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest opacity-60">{t('Bundle Rules')}</p>
                            <p className="text-[20px] font-black">{promo.b2cConfig?.bundleRules || t('Flash Sale')}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest opacity-60">{t('Store Price')}</p>
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 pt-6 border-t border-[#F0EDE8]">
                        <div className="flex items-center space-x-2 text-[12px] font-black text-gray-400 uppercase tracking-widest">
                           <Info size={16} />
                           <span>{t('Offer ends')} {formatDate(promo.endDate)}</span>
                        </div>
                        ...
                        >
                          ...
                          <span>{promo.isActiveInMyStore ? t('Deactivate') : t('Activate in Store')}</span>
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        {/* Feedback Modal */}
        {ratingData.id && (
          <div className="fixed inset-0 z-[70] !m-0 flex items-center justify-center bg-[#2C1810]/65 p-4 backdrop-blur-md">
            <div className="w-full max-w-[560px] overflow-hidden rounded-[28px] border border-[#E0DBD5] bg-white shadow-2xl animate-in zoom-in-95 duration-300">
              <div className="border-b border-[#F0EDE8] bg-white px-8 pb-6 pt-8">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-[#E0DBD5] bg-[#F8F7F5] text-[#3D2B1F]">
                      <Trophy size={24} />
                    </div>
                    <div>
                      <h3 className="text-[26px] font-black leading-tight tracking-tight text-[#2C1810]">{t('Campaign Impact')}</h3>
                      <p className="mt-1 text-[13px] font-semibold text-gray-500">{t('Report units sold and your store feedback.')}</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setRatingData({ id: null, score: 0, feedback: '', unitsSold: '' })}
                    className="rounded-full p-2 text-gray-400 transition-colors hover:bg-[#F8F7F5] hover:text-[#2C1810]"
                    aria-label="Close impact form"
                  >
                    <X size={20} />
                  </button>
                </div>
              ...
              <form onSubmit={handleRateSubmit} className="space-y-6 p-8">
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-[1fr_150px]">
                    <div className="space-y-2">
                      <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">{t('Total Units Sold')}</label>
                      <div className="relative">
                        ...
                        <input 
                          ...
                          min="1"
                          placeholder="e.g. 250"
                          className="w-full rounded-[18px] border border-[#E8E2DB] bg-[#FAFAF9] py-4 pl-12 pr-5 text-[15px] font-black text-[#2C1810] outline-none transition-all placeholder:text-gray-400 focus:border-[#3D2B1F] focus:bg-white focus:ring-4 focus:ring-[#3D2B1F]/5"
                    ...
                    <div className="rounded-[18px] border border-amber-100 bg-amber-50 px-4 py-4">
                      <div className="flex items-center gap-2 text-amber-700">
                        <Coins size={17} />
                        <p className="text-[10px] font-black uppercase tracking-widest">{t('Est. Points')}</p>
                      </div>
                      <p className="mt-2 text-[24px] font-black leading-none text-[#2C1810]">{estimatedReward.toLocaleString()}</p>
                    </div>
                  </div>

                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">{t('Recommendation Score')}</label>
                      <span className="rounded-full bg-[#3D2B1F] px-3 py-1 text-[12px] font-black text-white">{impactScore}/10</span>
                    </div>
                    <div className="rounded-[22px] border border-[#E8E2DB] bg-[#F8F7F5] p-5">
                      <input 
                      ...
                      />
                      <div className="mt-3 flex justify-between text-[11px] font-black uppercase tracking-widest text-gray-400">
                        <span>{t('Poor')}</span> 
                  ...
                  <div className="space-y-2">
                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">{t('Private Comments')}</label>
                    <textarea 
                      placeholder={t('Share notes on pricing, demand, stock movement, or logistics...')}
                      className="h-28 w-full resize-none rounded-[20px] border border-[#E8E2DB] bg-[#FAFAF9] p-5 text-[14px] font-bold text-[#2C1810] outline-none transition-all placeholder:text-gray-400 focus:border-[#3D2B1F] focus:bg-white focus:ring-4 focus:ring-[#3D2B1F]/5"
                  ...
                  <div className="flex flex-col-reverse gap-3 pt-2 sm:flex-row">
                    <button type="button" onClick={() => setRatingData({ id: null, score: 0, feedback: '', unitsSold: '' })} className="rounded-[16px] bg-gray-100 px-5 py-3.5 text-[13px] font-black uppercase tracking-widest text-gray-600 transition-colors hover:bg-gray-200">{t('Dismiss')}</button>
                    <button type="submit" className="flex-1 rounded-[16px] bg-[#3D2B1F] px-6 py-3.5 text-[13px] font-black uppercase tracking-widest text-white shadow-lg shadow-[#3D2B1F]/20 transition-all hover:bg-[#2C1810] active:scale-[0.98]">{t('Submit Impact')}</button>
                  </div>
                </form>
            </div>
          </div>
